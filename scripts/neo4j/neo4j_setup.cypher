...
// ==========================================
// 10. Section: Finance Domain
// ==========================================
MERGE (cfin1:Concept {name: "Credit Scoring"})
  ON CREATE SET cfin1.category = "finance", cfin1.definition = "Évaluation du risque de défaut d'un emprunteur"
MERGE (cfin2:Concept {name: "Look-Ahead Bias"})
  ON CREATE SET cfin2.category = "finance", cfin2.definition = "Biais d'utilisation d'informations futures non disponibles au moment de la prédiction"
MERGE (cfin3:Concept {name: "Sharpe Ratio"})
  ON CREATE SET cfin3.category = "finance", cfin3.definition = "Mesure de la rentabilité d'un actif par rapport à sa volatilité"

MERGE (tfin1:Tool {name: "SHAP"}) ON CREATE SET tfin1.category = "machine_learning"

MERGE (pfin1:Procedure {title: "Train Interpretable Credit Scorecard"})
  ON CREATE SET pfin1.domain = "finance",
                pfin1.steps = "[\"Nettoyer les variables financières\", \"Appliquer une Régression Logistique\", \"Extraire les coefficients et les traduire en grille de points score\"]"

MERGE (pfin2:Procedure {title: "Stationnariser serie financiere"})
  ON CREATE SET pfin2.domain = "finance",
                pfin2.steps = "[\"Appliquer le test ADF\", \"Si p-value > 0.05, calculer les rendements log log(price).diff()\", \"Valider la stationnarité sur les résidus\"]"

MERGE (dfin1:Decision {id: "finance-model-selection"})
  ON CREATE SET dfin1.question = "Comment modéliser le risque financier ?",
                dfin1.context = "finance",
                dfin1.decision_tree = "{\"root\": \"Exigence d'explication\", \"branches\": [{\"condition\": \"Réglementation stricte demandant explications simples\", \"action\": \"Utiliser Régression Logistique avec Scorecard\"}, {\"condition\": \"Performance maximale avec explication locale via attribution\", \"action\": \"Utiliser XGBoost + SHAP\"}]}"

// Relations Finance
MATCH (p1:Procedure {title: "Train Interpretable Credit Scorecard"}), (c1:Concept {name: "Credit Scoring"}), (t1:Tool {name: "SHAP"})
MERGE (p1)-[:REQUIRES]->(c1)
MERGE (p1)-[:USES_TOOL]->(t1);

MATCH (p2:Procedure {title: "Stationnariser serie financiere"}), (c2:Concept {name: "Look-Ahead Bias"}), (t2:Tool {name: "statsmodels"})
MERGE (p2)-[:AVOIDS]->(c2)
MERGE (p2)-[:USES_TOOL]->(t2);


// ==========================================
// 11. Section: E-commerce Domain
// ==========================================
MERGE (ceco1:Concept {name: "Modèle RFM"})
  ON CREATE SET ceco1.category = "ecommerce", ceco1.definition = "Segmentation basée sur la Récence, Fréquence et Montant d'achat"
MERGE (ceco2:Concept {name: "Churn Client"})
  ON CREATE SET ceco2.category = "ecommerce", ceco2.definition = "Prédiction du taux d'attrition des utilisateurs"

MERGE (peco1:Procedure {title: "Segmenter avec le modele RFM"})
  ON CREATE SET peco1.domain = "ecommerce",
                peco1.steps = "[\"Calculer Recency, Frequency et Monetary par client\", \"Normaliser les colonnes (StandardScaler)\", \"Appliquer KMeans pour segmenter en clusters\"]"

MERGE (peco2:Procedure {title: "Entrainer un modele de Churn"})
  ON CREATE SET peco2.domain = "ecommerce",
                peco2.steps = "[\"Définir la période d'inactivité (ex: 90 jours)\", \"Gérer le déséquilibre des classes\", \"Entraîner un classifieur de type Gradient Boosting\"]"

MERGE (deco1:Decision {id: "ecommerce-pricing-strategy"})
  ON CREATE SET deco1.question = "Comment optimiser les prix en E-commerce ?",
                deco1.context = "ecommerce",
                deco1.decision_tree = "{\"root\": \"Volume de données de ventes\", \"branches\": [{\"condition\": \"Faible historique ou nouveautés\", \"action\": \"Règles expertes métier basées sur les coûts\"}, {\"condition\": \"Historique transactionnel complet\", \"action\": \"Estimer l'élasticité-prix de la demande par régression log-log\"}]}"

// Relations E-commerce
MATCH (p3:Procedure {title: "Segmenter avec le modele RFM"}), (c3:Concept {name: "Modèle RFM"}), (t3:Tool {name: "scikit-learn"})
MERGE (p3)-[:REQUIRES]->(c3)
MERGE (p3)-[:USES_TOOL]->(t3);

MATCH (p4:Procedure {title: "Entrainer un modele de Churn"}), (c4:Concept {name: "Churn Client"}), (t4:Tool {name: "XGBoost"})
MERGE (p4)-[:REQUIRES]->(c4)
MERGE (p4)-[:USES_TOOL]->(t4);


// ==========================================
// 12. Section: Medical Domain
// ==========================================
MERGE (cmed1:Concept {name: "Données censurées"})
  ON CREATE SET cmed1.category = "medical", cmed1.definition = "Données où l'événement d'intérêt (ex: décès, récidive) n'a pas été observé pendant l'étude"
MERGE (cmed2:Concept {name: "Recall Médical"})
  ON CREATE SET cmed2.category = "medical", cmed2.definition = "Priorité clinique donnée au fait de ne rater aucun malade (minimiser les faux négatifs)"

MERGE (tmed1:Tool {name: "lifelines"}) ON CREATE SET tmed1.category = "survival"

MERGE (pmed1:Procedure {title: "Kaplan-Meier Survival Estimation"})
  ON CREATE SET pmed1.domain = "medical",
                pmed1.steps = "[\"Charger données avec colonnes Temps et Événement\", \"Instancier KaplanMeierFitter de lifelines\", \"Ajuster et tracer la courbe de survie\"]"

MERGE (pmed2:Procedure {title: "Ajuster un modele de Cox"})
  ON CREATE SET pmed2.domain = "medical",
                pmed2.steps = "[\"Encoder les variables catégorielles\", \"Instancier CoxPHFitter de lifelines\", \"Ajuster et analyser les hazard ratios\"]"

MERGE (dmed1:Decision {id: "medical-metric-selection"})
  ON CREATE SET dmed1.question = "Quelle métrique de performance pour le diagnostic médical ?",
                dmed1.context = "medical",
                dmed1.decision_tree = "{\"root\": \"Gravité de la maladie\", \"branches\": [{\"condition\": \"Maladie grave (taux de survie lié à la détection)\", \"action\": \"Maximiser le Recall / Sensibilité\"}, {\"condition\": \"Diagnostic screening global\", \"action\": \"Optimiser ROC-AUC ou F1-weighted\"}]}"

// Relations Medical
MATCH (p5:Procedure {title: "Kaplan-Meier Survival Estimation"}), (c5:Concept {name: "Données censurées"}), (t5:Tool {name: "lifelines"})
MERGE (p5)-[:REQUIRES]->(c5)
MERGE (p5)-[:USES_TOOL]->(t5);

MATCH (p6:Procedure {title: "Ajuster un modele de Cox"}), (c6:Concept {name: "Données censurées"}), (t6:Tool {name: "lifelines"})
